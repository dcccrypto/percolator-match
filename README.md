...
**The LP PDA must sign** `InitVamm` (PERC-321) — otherwise anyone could initialise an
uninitialised, program-owned context account with attacker-controlled parameters (fees,
spread, inventory limits) and lock out the intended LP via the one-time-init guard.
